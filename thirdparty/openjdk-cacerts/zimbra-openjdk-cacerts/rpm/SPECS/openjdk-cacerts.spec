...
if [ "$1" -ge "2" ]; then
  if [ -x /opt/zimbra/bin/zmcertmgr ]; then
    ...
    for dir in /opt/zimbra/.saveconfig/zimbra-openjdk-cacerts-1.0.[5-6]*; do
        ...
        for cert in `/opt/zimbra/common/bin/keytool -list -keystore $dir/cacerts.*  -storepass changeit | grep trustedCertEntry |  grep -v thirdparty | grep -v 'tmp/rhel' | grep -Eo "^[^,]*"`;do
